...
<flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-950">
    ...
    <flux:navbar class="-mb-px max-lg:hidden">
        ...
        <flux:dropdown>
            <flux:navbar.item icon-trailing="chevron-down" icon="map">Known
                                                                      Places
            </flux:navbar.item>

            <flux:navmenu>
                <flux:navmenu.item href="{{ route('known-places.create') }}" icon="plus">Create
                </flux:navmenu.item>
                <flux:navmenu.item href="#" icon="arrow-down-tray" wire:navigate>Import</flux:navmenu.item>
            </flux:navmenu>
        </flux:dropdown>
        <flux:navbar.item icon="magnifying-glass-plus" :href="route('dashboard')"
                          :current="request()->routeIs('dashboard')"
                          wire:navigate>
            {{ __('Scenarios') }}
        </flux:navbar.item>
        <flux:navbar.item icon="document-chart-bar" :href="route('dashboard')"
                          :current="request()->routeIs('dashboard')"
                          wire:navigate>
            {{ __('Reports') }}
        </flux:navbar.item>
